...
bool flvParser::parserAudioData(flvTag & audioTag, timedPacket & outPkt)
{
	...
	do
	{
		//仅仅读数据
		DataPacket pkt;
		pkt.size = audioTag.dataSize;
		pkt.data = new char[pkt.size];
		if (pkt.size != m_fileHander->readFile(pkt.size, pkt.data)) {
			result = false;
			break;
		}
		outPkt.data.push_back(pkt);
...
bool flvParser::parserVideoData(flvTag & videoTag, timedPacket & outPkt)
{
	...
	{
		DataPacket pkt;
		outPkt.timestamp = videoTag.timestamp;
		outPkt.packetType = flv_pkt_video;
		tmpBuf = new char[videoTag.dataSize];
		if (videoTag.dataSize != m_fileHander->readFile(videoTag.dataSize, tmpBuf)) {
			result = false;
			break;
		}
			pkt.size = videoTag.dataSize;
			pkt.data = new char[videoTag.dataSize];
			memcpy(pkt.data, tmpBuf, videoTag.dataSize);
			outPkt.data.push_back(pkt);
	} while (0);
	if (tmpBuf)
	{
